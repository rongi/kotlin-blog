...
It is pretty common to implement data layers with Rx. But for some time a question was bothering me: "Is it possible to implement UI logic with Rx? Is it possible to write a presenter entirely in Rx?" Rx was designed to handle tasks exactly like that --- programming fundamentally asynchronous processes, but you can never tell if the solution can survive countless edge cases and pressure of real-life requirements unless you try it in a production application.

So, about a year ago I had this opportunity to try this idea in a real production project. I started by implementing a little bit of a presenter with Rx but it quickly became obvious that the approach is so powerful, it produces so simple, easy to understand and reason about code, that it's benefits outweighed even the necessity to learn a completely new programming paradigm. So I'd ended up implementing whole UI layer of the app with Rx and in this article I will explain how I did it.

## Presenter is a function

I started by having plain old stateful presenter. While reimplementing it with Rx, I removed state by state from it as they become redundant. Eventually it became evident that my presenter is trying to take this form:
...
It transforms them into a `ViewModel` object, which is just a collection of view states, like text field values or view visibilities. In the form of observables again:
...
View states come in the form of observables. So, all we need to render them is subscribe to that observables and map values emitted by them to corresponding properties of the views:
...
Note how we've just reused `articles` observable to build a new view model. This trick and creating chains for each view state instead of creating single mega-chain are keys to make this solution functional.
