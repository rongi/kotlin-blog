...
I started with plain old presenter and just tried to implement it with Rx. But it quickly became obvious that, then you think in Rx, all the OOP ceremony that comes with traditional presenters is no longer needed. It became obvious that my presenter is trying to take this form:
...
It takes a collection of interaction events, like button clicks or text field changes:
...
To render one of those, just subscribe and map it's value to the view:
...
This is how activity implementation usually looks like:
...
The `render()` function just goes through each of the view models and renders it:
...
This function is the heart of our presentation layer. It owns all the complexity and all non-trivial presentation logic. How is this function implemented? Let's see that on an example of this activity:
...
And this is just a rule how to transform `updateButtonClicks` observable into `articles` observable, using `articlesService` as a dependancy.
...
No, just build a new observable! Feel free to use existing ones as building blocks. Like this:
...
Now we have two rules. And this is how the rest of our work will look like. We identify which view state we want to implement next and then we write a rule how to transform user input and other dependencies into this view state. We can use already created observables to implement new ones.
