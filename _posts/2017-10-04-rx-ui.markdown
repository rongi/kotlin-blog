...
It is pretty common to implement data layers with Rx --- just glue up some requests together with `merge()` or `concat()` and subscribe to the result. It's no so common to use Rx in presentation layer (presenters or view models).

Yet presentation layers are exactly the type of systems Rx is designed for: it's a complex fundamentally asynchronous system.



But even if it's suitable in theory, will it work in real life? Will it survive pressure of real life requirements? I had this question in my head for some time, so when an opportunity to check it arise, I've tried.
...
I started my journey with a traditional `class Presenter`, and just tried to implement the logic inside with Rx, step by step. It quickly became obvious though, that the class is no longer needed, it became clear that my presenter is trying to take this form:
...
A class became a function, and this function has very simple meaning: it transforms a set of input streams, like user interaction and system events into view states.

[Image: clicks -present()-> view state]

The input streams are in the form of observables of course. The most common example will be observables containing button clicks and text field changes:
...
`present` function transforms them into a collection of view states: text field values, view visibilities, etc.:
...
It's just a collection of observables. To render, just subscribe and map emitted values to view properties:
...
And, to sum this up, this is how activity usually looks like:
...
So, as we can see presenter became a simple transform function. It transforms observables with user input into observables with view states.
...
Now we have two rules. Rule after a rule, and our app is fully implemented.
