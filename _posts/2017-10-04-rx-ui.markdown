...
I started my journey with a traditional `class Presenter`, which, as you know, incorporates all the presentation logic, and just tried to implement it's guts with Rx. It was a surprise when it became obvious suddenly, that the class itself is no longer needed. It looked like presenter is trying to take this form:
...
What do I mean by "an observables with user interaction events"? An observable that emits an item each time a button is clicked is the most common example. Another example is an observable that emits text field's content each time user types something in.
...
`present()` function then transforms these streams into a collection of view states, and returns these view state packed into a `ViewModel` class. `ViewModel` is just a shallow container for view states observables:
...
To render, we just subscribe to each of these observables and map their values to view properties:
...
`onCreate()` is like a `main()`, but for an activity. It's a main entry point of the software unit. Having this in mind, we may notice that the code above resembles this:
