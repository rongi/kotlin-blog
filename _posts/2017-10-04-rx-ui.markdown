...
As an input it takes a bunch of observables with user interaction events. An example of such an observable will be an observable that emits an item each time a button is clicked:
...
As an output it return an instance of `ViewModel` object, which is just a container for a bunch of observables with view states. View states are values that can be mapped to some view property: view visibilities or label texts will be the most common examples here:
...
So, roughly speaking, `present()` is just a function that transforms clicks into the stuff we see on the screen:
