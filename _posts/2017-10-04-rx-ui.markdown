---
layout: post
title: "Functional Reactive architecture for Android applications or how to build your presenter with RxJava"
excerpt: "Rx UI"
...
It is pretty common to implement data layers with Rx. Glue some requests together with `merge()` or `concatMap()` and subscribe to the result in the presenter. But is it possible to implement presenter itself with Rx? If you think about it, Rx was designed to handle tasks exactly like that --- programming fundamentally asynchronous processes. But the only way to tell if the solution will survive countless edge cases and pressure of real-life requirements is to try it in this conditions.

So, about a year ago got an opportunity to try this idea in a real-life production project. I started by implementing just a little bit of the presenter with Rx. It quickly became obvious though, that the approach is so powerful that it's benefits out weight even the necessity to learn completely new programming paradigm. I'd ended up implementing whole UI layer of the app with Rx. It was a truly exciting journey full of insights. The architecture evolved in the process, and here I want to share how it looked in it's final form.
...
What are the data streams we are talking about?

It takes as an input a bunch of user interaction events, like button clicks or text field changes. All of them are in the form of observables:
...
So, presenter became a simple transform function. It takes a bunch of observables and transform them into another bunch of observables. As you'll see further, it can do anything traditional OOP presenter can. It does the same thing, fulfills the same role, but in a functional way.

## Rendering view state

`present()` function produces a collection of view states which are just observables. To render them, we just subscribe and then map values to the corresponding properties of the views:
