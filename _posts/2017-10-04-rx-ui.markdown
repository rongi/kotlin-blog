...
I started with plain old presenter and just tried to implement it with Rx. But it quickly become obvious that, then you think in Rx, all the OOP ceremony that comes with traditional presenters is no longer needed. It become obvious that my presenter is trying to take this form:
...
`present()` function produces a collection of view states, which are observables. To render them, just subscribe to these observables:
